...
<template>
  <div class="flex-1 conversations-list overflow-x-hidden overflow-y-scroll color-base">
    <template v-for="(item, index) in conversationStore.conversationsList" :key="index">
      <div
        v-if="index === 0 && (item.fixed_top ?? false)"
      ...
      >
        ...
        <div
          class="w-14px h-14px icon-button" :class="showFixedTop ? 'i-carbon-chevron-down' : 'i-carbon-chevron-up'"
          data-cursor="block"
        />
      </div>
      <div
      ...
      >
        ...
        <div
          class="icon-button w-14px h-14px"
